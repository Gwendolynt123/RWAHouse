...
contract RWAHouse is SepoliaConfig {
    ...
    function storePropertyInfo(
        ...
        properties[userAddress] = PropertyInfo({country: country, city: city, valuation: valuation, exists: true});
    ...
    function countryQueryCallback(uint256 requestId, bool result, bytes[] memory signatures) public {
    ...
    function cityQueryCallback(uint256 requestId, bool result, bytes[] memory signatures) public {
        require(decryptionRequests[requestId].isPending, "Request not pending");
        FHE.checkSignatures(requestId, signatures);
    ...
    function valuationQueryCallback(uint256 requestId, bool result, bytes[] memory signatures) public {
        require(decryptionRequests[requestId].isPending, "Request not pending");
        FHE.checkSignatures(requestId, signatures);
